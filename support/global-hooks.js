...
function userInterfaceHooks(globals) {
  ...
  Before(async () => {
    ...
    const shoeSizesTable=[36,38,40,42,44,46,48]
    //Registering handler for alerts/dialogs
    page.on('dialog', dialog => { dialog.dismiss(); });
    global.config.inputs.generatedEmail= `josssantos+${Math.floor(Math.random() * 1000000)}@hotmail.com`
    global.config.inputs.shoeSize= Math.floor(Math.random() * 6)
